...
# --------------------------------------------------------------------------- #
# Annotation-export -> merged-export conversion
# --------------------------------------------------------------------------- #
#
# Label Studio target/export cloud storage writes one JSON object *per annotation*
# (`{id, result, task: {id, data}, was_cancelled, updated_at, ...}`), whereas the
# manual Export button — and therefore `CaptureDataset.load` / `_parse_task` — wants
# one object *per task* with annotations nested (`{id, data, annotations: [{result}]}`).
# These helpers fold the former into the latter so the bucket of exports can drive the
# whole training stack through the single `data/captures/label-studio.json` path.


def export_task_from_annotation(ann: dict) -> dict | None:
    """One per-annotation export object -> a merged-export *task* dict.

    Returns None for cancelled or empty annotations (Label Studio "skips" carry no
    usable labels) and for objects missing the embedded task / image reference. The
    task `id` is taken from the embedded **task** (stable across re-labels), not the
    annotation id, so `CaptureSample.task_id` stays consistent across re-exports.
    """
    if ann.get("was_cancelled"):
        return None
    result = ann.get("result") or []
    task = ann.get("task") or {}
    if not result or "id" not in task or "data" not in task:
        return None
    return {
        "id": task["id"],
        "data": task["data"],
        "annotations": [{"result": result, "updated_at": ann.get("updated_at") or ""}],
    }


def build_export_from_annotations(annotations: Iterable[dict]) -> list[dict]:
    """Merge per-annotation export objects into a deduped, task-keyed export array.

    Multiple annotations can target one task (a re-label leaves the old annotation
    file in the bucket — exports are additive, never pruned), so we keep only the
    most recently `updated_at` annotation per task id. Output is sorted by task id
    for stable diffs.
    """
    by_task: dict[int, dict] = {}
    for ann in annotations:
        task = export_task_from_annotation(ann)
        if task is None:
            continue
        tid = int(task["id"])
        updated = task["annotations"][0]["updated_at"]
        if tid not in by_task or updated > by_task[tid]["annotations"][0]["updated_at"]:
            by_task[tid] = task
