...
- **Corner-label mode (corner-capture)** — corner-only labels in the unified flat `data/` store (`chessvision/data/corner_capture.py`) fed by phone photos dumped in `data/source/inbox/` and labelled in the web app (`--corners-root data`): a date-grouped photo browser → the existing corner-marking grid (+ "Predict") → sticky Board tag → `data/store/<relpath>` + `data/labels.jsonl`. EXIF is normalized **on label** so display/storage/training share one pixel frame. Deliberately separate from the capture set (corner-only, no FEN/Label-Studio). Folds into corner training via `--corners-root` (default on) as the `cds_*` eval + selection metric. The point is **viewpoint diversity** — see `docs/corner-capture-mode.md`.
- **Phase 4 (image→FEN) — per-board eval, 2026-05-29** (`scripts/eval_per_board_captures.py`): on held-out capture sessions, **end-to-end == GT-corner ceiling exactly** on both boards, i.e. the **corner model is NOT the bottleneck** — the keypoint head's per-piece classification is. staunton-56mm: localization 0.962, **class_acc 0.905**, board_exact 13.7%; cheap-30mm: 0.973 / **0.809** / 0%. board_exact is low purely from compounding (~30 pieces × ~94% conditional). So the lever for whole-board accuracy is per-piece **class accuracy** (the model that runs on captures is the Keypoint R-CNN = the Faster R-CNN v2 trunk + grafted contact-keypoint head, `runs/keypoint_captures/best.pt`; the standalone box-only baseline `runs/detector/best.pt` is only scored on ChessReD). Per-class confusion (`--confusion`, held-out Staunton): the gap is almost all **dark/black pieces** (they're missed more *and* misclassified more) plus **tall-piece K/Q/B shape confusion** (worst: black-queen→black-king 17/51); white pieces are ~solved (pawn/rook/knight 1.00). Counts are color-balanced, so it's an appearance/legibility problem, not class imbalance → the cheap high-leverage fix is **lighting + angle diversity on the Staunton set** (position-label tool), favouring diverse real captures over synthetic renders as the next step. Caveat: one held-out session, so partly that session's lighting.
...
# Corner-label mode: dump phone photos in data/source/inbox/, then label corners in-app.
...
uv run python scripts/sync_captures.py up --local data/store --prefix corners  # back up labels+images
```
...
Read a chess position from a single photo and output **FEN**. The hard requirement is **generalization across many boards, piece sets, and environments** — not making one sample setup work. Overfitting to a single domain is the explicit failure mode being designed against (the "prior project" referenced throughout is the student repo **TFG-Diego** (`github.com/Deinigu/TFG-Diego`), which the user tried and which failed for them — its **Hough-line board detection** broke; this is the origin of the no-Hough anti-pattern. It is **not** the user's own work and **not** ChessReD; its reported accuracy is likely **inflated** (student project), so it is not a baseline to beat).

> Licensing constraint: this is a clean reimplementation. Do **not** copy from the prior (non-MIT) project's source (TFG-Diego). Verify the license of every dataset and reference system before use (`chesscog`, ChessReD, LiveChess2FEN, Roboflow sets).
