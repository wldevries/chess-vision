...
def _app(predictor=None):
    from chessvision.capture.app import create_app

    return create_app([], "out_unused", predictor=predictor)
...
def test_live_mode_off_by_default():
    from fastapi.testclient import TestClient

    client = TestClient(_app(predictor=None))
    assert client.get("/api/live/available").json() == {"available": False}
    resp = client.post(
        "/api/live/predict",
        files={"image": ("f.jpg", _jpeg(), "image/jpeg")},
        data={"corners": json.dumps(SQUARE_CORNERS)},
    )
    assert resp.status_code == 503


def test_live_predict_returns_per_orientation_fens():
    from fastapi.testclient import TestClient

    stub = _StubPredictor([_center("e2")], labels=[1], scores=[0.9])
    client = TestClient(_app(predictor=stub))
...
def test_live_predict_rejects_bad_corners():
    from fastapi.testclient import TestClient

    stub = _StubPredictor([], labels=[], scores=[])
    client = TestClient(_app(predictor=stub))
    resp = client.post(
        "/api/live/predict",
        files={"image": ("f.jpg", _jpeg(), "image/jpeg")},
        data={"corners": json.dumps([[0, 0], [1, 1]])},  # only 2 points
    )
